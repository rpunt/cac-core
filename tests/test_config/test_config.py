...
def temp_config_file():
    ...
    with tempfile.NamedTemporaryFile(
        ...
        config_data = {
            ...
            "feature_flags": {"enable_logging": True, "debug_mode": False},
        }
...
class TestConfig:
    ...
    def test_load_existing_config(self, temp_config_file):
        ...
        )  # Ensure we have a string path, not bytes
    ...
    def test_set_nested_value(self):
        ...
        assert (
            config.config["nested"]["key"] == "nested-value"
        )  # Changed from data to config
    ...
    def test_update_nested_dict(self):
        ...
        config = Config("test-app")
        config.update({"level1": {"level2": {"level3": "value"}}})
